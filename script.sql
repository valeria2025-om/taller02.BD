...
SELECT DISTINCT c.Titulo, intr.Nombre AS Interprete
FROM Cancion c
JOIN Interpretacion i ON c.Id = i.IdCancion
JOIN Interprete intr ON i.IdInterprete = intr.Id
WHERE c.Titulo IN ('Candilejas', 'Malagueña');

SELECT intr.Nombre,
       COUNT(DISTINCT cc.IdCancion) AS CancionesCompuestas,
       COUNT(DISTINCT i.IdCancion) AS CancionesInterpretadas
FROM Interprete intr
JOIN Compositor comp ON CHARINDEX(intr.Nombre, comp.Nombre) > 0
LEFT JOIN Cancion_Compositor cc ON comp.Id = cc.IdCompositor
LEFT JOIN Interpretacion i ON intr.Id = i.IdInterprete
